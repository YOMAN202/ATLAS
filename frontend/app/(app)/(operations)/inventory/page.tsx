...
export default function InventoryDashboardPage() {
  ...
  const detail = useApi(
    () => api.inventory.detail(role, { page, page_size: pageSize }),
    [role, page],
  );

  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-headline font-semibold text-ink-primary">Inventory</h1>

      {summary.status === "loading" && <DashboardLoading />}
      {summary.status === "error" && <DashboardError error={summary.error} />}
      {summary.status === "ready" && (
        <>
          <p className="text-xs text-ink-muted">As of {summary.data.latest_snapshot_date ?? "—"}</p>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
            <KpiCard
              label="On Hand (units)"
              value={formatNumber(summary.data.total_quantity_on_hand)}
            />
            <KpiCard
              label="Inventory Value"
              value={formatCurrency(summary.data.total_inventory_value)}
            />
            ...
            <KpiCard
              ...
              value={summary.data.days_of_supply?.toFixed(1) ?? "—"}
            />
          </div>
          <KpiCard
            label="Overstock Value"
            value="—"
            note={summary.data.overstock_value_note}
            className="md:w-1/3"
          />
        </>
      )}
      ...
      <div>
        <h2 className="mb-2 text-sm font-medium text-ink-secondary">
          Snapshot Detail (most recent first)
        </h2>
