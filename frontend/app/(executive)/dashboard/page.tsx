...
export default function ExecutiveDashboardPage() {
  ...
  const query = useApi(
    () => api.executive.summary(role, { date_from: dateFrom || undefined, date_to: dateTo || undefined }),
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm">
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="rounded-md border border-slate-300 px-2 py-1 dark:border-slate-700 dark:bg-slate-900" />
          <span className="text-slate-400">to</span>
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="rounded-md border border-slate-300 px-2 py-1 dark:border-slate-700 dark:bg-slate-900" />
        </div>
      </div>
      ...
      {query.status === "ready" && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
            ...
            <KpiCard label="Fulfillment Rate" value={formatPercent(query.data.order_fulfillment_rate)} />
          </div>

          <KpiCard label="Cost to Serve" value="—" note={query.data.cost_to_serve_note} className="md:w-1/3" />
