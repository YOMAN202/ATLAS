...
**ADR-023: Pluggable claim-drafting client — the CI-blocking gate depends on the deterministic verifier, not a live LLM call**
*Context:* Phase 8's analytics copilot requires an LLM to draft structured claims (`docs/phase8-grounding-spec.md` §1). No LLM API key exists anywhere in this project's `.env` or configuration at the time of implementation, and no LLM SDK was previously a dependency. The authorized architecture requires the verification harness to be a mandatory, CI-blocking release gate — a gate that depends on network access to a paid external API on every CI run would be flaky, slow, and costly, none of which this project has accepted for any other test in its 259-test suite.
*Decision:* Claim drafting is built behind an abstract interface, `ClaimDraftingClient.draft_claims(question, tool_results) -> list[RawClaim]`, with two implementations: `AnthropicClaimDraftingClient` (the real production path, using the `anthropic` SDK and `settings.anthropic_api_key`, raising a clear configuration error — never a silent fallback — if the key is unset) and `FixtureClaimDraftingClient` (a deterministic, hand-authored set of realistic and deliberately-flawed claim drafts, keyed by question, used by every test and by the CI-blocking eval harness). The verifier, renderer, refusal logic, and citation generation are all pure, deterministic Python with no network dependency, and are the components the CI-blocking threshold in `docs/phase8-grounding-spec.md` §6 actually gates on.
*Rationale:* The grounding claim this whole harness exists to prove is "an unverifiable claim is never shown to the user" — that is a property of the *verifier*, not of any specific LLM's behavior on any specific day. Testing the verifier against a fixed, adversarial, version-controlled set of claim drafts (including drafts that deliberately misstate a value, cite the wrong source, or invert a comparison) proves the property the harness is actually responsible for: catching bad claims when they occur, regardless of what produces them. A live-LLM-dependent CI gate would conflate two different failure modes — "the verifier has a bug" and "the model said something odd today" — and would make the release gate flaky for reasons that have nothing to do with the platform's own correctness, which is exactly the kind of external dependency every other module in this platform was built without.
*Alternatives considered:* (a) Require a live API key and skip the harness in CI when absent — rejected: a release gate that silently doesn't run when a credential is missing is not a gate; (b) Record real LLM responses once via VCR-style cassettes and replay them in CI — a reasonable future refinement once a key exists, but functionally equivalent to the fixture approach for what the CI gate needs to prove today, and adds a recording/replay dependency this project doesn't otherwise have; (c) Mock the LLM call to always return a hardcoded "good" claim — rejected: would not exercise the verifier's actual job of catching a *bad* claim, which is the property that matters most (per the grounding spec's own emphasis: refusal-on-bad-evidence is weighted more heavily than raw helpfulness).
*Consequences:* The real `AnthropicClaimDraftingClient` is implemented and ready to use once `ANTHROPIC_API_KEY` is configured, but has not been exercised end-to-end against a live model in this pass — that is explicitly disclosed in the verification results report, not implied to have been tested. Extending the eval harness with real, recorded model outputs (alternative (b) above) is the natural next step once a key is available, and does not require re-architecting anything specified here — only adding a third `ClaimDraftingClient` implementation.

**ADR-024: Gemini as the primary claim-drafting/agentic provider — configuration-driven provider selection, Anthropic kept as an optional future provider**
*Context:* ADR-023 defined `ClaimDraftingClient` as a pluggable abstraction specifically so the CI-blocking gate never depends on a live LLM, and specifically so a real provider could be added later without touching the verifier/renderer/refusal pipeline. Phase 8.1 (the chat interface) requires a real, live LLM to power a genuine conversational agent — not just single-shot claim drafting but *tool selection*, since a chat interface doesn't know in advance which of the six retrieval tools a question needs. At the point Phase 8.1 was authorized, no `ANTHROPIC_API_KEY` existed in this environment; a `GEMINI_API_KEY` (Google AI Studio, free tier) was chosen as the credential to actually exercise end-to-end.
*Decision:* Gemini becomes the default, primary live provider (`settings.copilot_llm_provider = "gemini"`), selected through a small configuration-driven dispatcher (`app/copilot/provider.py::run_copilot_pipeline` / `get_claim_drafting_client`) rather than a hardcoded import. `AnthropicClaimDraftingClient` (ADR-023) is left exactly as it was — a declared, unimplemented-but-typed `ClaimDraftingClient` — and remains selectable via `copilot_llm_provider = "anthropic"` for the single-shot claim-drafting interface, but has no live agentic (tool-selecting) implementation; selecting it for the chat endpoint raises a clear `ValueError` rather than silently falling back to Gemini. A new module, `app/copilot/gemini_agent.py`, holds both `GeminiClaimDraftingClient` (fulfilling the same `ClaimDraftingClient` ABC as the Anthropic client, for interface parity) and `run_agentic_pipeline` (the live multi-turn tool-selecting loop that actually powers `GET /api/v1/copilot/ask`).
*Rationale:* The provider abstraction ADR-023 already specified is exactly what makes this swap cheap and safe: zero changes to `verifier.py`, `refusal.py`, `renderer.py`, `claims.py`, `citations.py`, or any of the 50 CI-blocking tests in `backend/tests/copilot/` that exercise them. Gemini's function-calling shape (the `google-genai` SDK's Interactions API — `client.interactions.create(..., tools=[...])`, function_call/function_result steps) differs from Anthropic's tool-use content-block shape, but that difference is entirely contained inside `gemini_agent.py`; the claim schema it drafts into (`FactClaim`/`ComparisonClaim`/`DerivedClaim`) and the verification it's checked against are identical regardless of which provider proposed them. This is the direct, intended payoff of building the pluggable interface *before* wiring a real provider, rather than the reverse.
*Alternatives considered:* (a) Hardcode Gemini as the only provider, deleting the Anthropic stub — rejected: the user explicitly asked for Anthropic to remain declared as an optional future provider, and the marginal cost of leaving an unused, already-typed stub in place is near zero; (b) Make `run_agentic_pipeline`'s tool-selecting loop itself part of the `ClaimDraftingClient` ABC (change the interface to accept an `HttpCaller` and do routing internally) — rejected: would force `AnthropicClaimDraftingClient` and every existing test/fixture built against the narrower single-shot `draft_claims(question, tool_results)` contract to change shape for a capability only the live chat path needs; keeping `run_agentic_pipeline` as a separate function outside the ABC, dispatched by `provider.py`, avoids touching the already-proven interface at all.
*Consequences:* `GeminiClaimDraftingClient.draft_claims` and `run_agentic_pipeline` are real, non-stub implementations (unlike `AnthropicClaimDraftingClient.draft_claims`, which still raises `NotImplementedError`) — this project now has one live-network-capable LLM integration and one declared-but-inert one. Provider-specific behavioral differences (function-calling shape, model naming, conversation-history handling, safety/refusal behavior) are documented separately in `docs/phase8-gemini-provider-notes.md` rather than folded into the provider-neutral architecture docs.
